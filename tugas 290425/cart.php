...
if (!empty($kategori_dipesan)) {
    ...
    $stmt = $pdo->prepare("
        SELECT * FROM produk 
        WHERE id NOT IN (SELECT produk_id FROM keranjang WHERE user_id = ?)
        ORDER BY RAND() 
        LIMIT 4
    ");
    $stmt->execute([$_SESSION['user_id']]);
...
<body>
                                ...
                                <?php foreach ($rekomendasi as $menu): ?>
                                    <div class="col-md-3 mb-3">
                                        <div class="card h-100 border-0 shadow-sm">
                                            <img src="images/<?= htmlspecialchars($menu['gambar']) ?>" 
                                                 class="card-img-top" 
    ...
    <script>
        document.querySelector('select[name="metode_pengambilan"]').addEventListener('change', function() {
            ...
            if (this.value !== 'dine-in') nomorMeja.querySelector('input').value = '';
            if (this.value !== 'delivery') alamatDelivery.querySelector('textarea').value = '';
        });
    </script>
</body>
</html>
